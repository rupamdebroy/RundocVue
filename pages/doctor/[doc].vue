    ...
    <div v-else-if="doctor" class="space-y-10">
      ...
      <div class="max-w-7xl mx-auto">
        <nav class="text-blue-700 text-sm">
          ...
          <span class="text-blue-700 font-medium">Find Doctors</span>
        </nav>
      </div>
      ...
      >
        ...
        <div class="py-0 rounded-2xl">
          <p class="text-gray-600 leading-relaxed">
            ...
            providing top-notch care, Dr. {{ doctor.name }} focuses on accurate
            diagnosis and effective treatment plans tailored to each patient's
            needs.
          </p>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto py-4">
        ...
        <div class="relative px-8">
          ...
          <button
            @click="scrollLeft"
            class="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-blue-700 text-white p-2 rounded-full shadow hover:bg-blue-700 transition"
          >
          ...
          <button
            @click="scrollRight"
            class="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-blue-700 text-white p-2 rounded-full shadow hover:bg-blue-700 transition"
          >
      ...
      <div class="bg-gray-100 py-8">
        <div class="max-w-7xl mx-auto px-4">
          ...
          >
            ...
            {{ doctor.name }}. Please check back later or choose another doctor.
          </div>
...
onMounted(async () => {
  try {
    console.log(
      "[doc].vue mounted - Store location:",
      doctorsStore.selectedLocation
    );

    if (!doctorsStore.selectedLocation || doctorsStore.doctors.length === 0) {
      console.warn("No location or doctors in store, redirecting to home");
    ...
    if (authStore.isAuthenticated) {
      try {
        // Simulate fetching user's recommendation
        // Example: const recoResponse = await axios.get(`/api/doctor/${doctorSlug.value}/recommendation`, { headers: { Authorization: `Bearer ${authStore.userInfo.sessionId}` } });
        // userRecommendation.value = recoResponse.data.userRecommendation || null;
        // recommendationPercentage.value = recoResponse.data.percentage || 0;
...
const recommendDoctor = async (isRecommended) => {
  ...
  userRecommendation.value = isRecommended;
  console.log(
    `Submitting recommendation ${isRecommended} for doctor ${doctorSlug.value}`
  );

  // Simulate API call to save recommendation
  try {
    // Example:
    // await axios.post(`/api/doctor/${doctorSlug.value}/recommend`, { isRecommended }, { headers: { Authorization: `Bearer ${authStore.userInfo.sessionId}` } });
...
// Other functions (rating, navigation, etc.) remain unchanged
const setUserRating = async (rating) => {
  if (!authStore.isAuthenticated) {
    sessionStorage.setItem("redirectAfterLogin", route.fullPath);
    navigateTo("/auth");
    return;
  ...
  console.log(`Submitting rating ${rating} for doctor ${doctorSlug.value}`);
  averageRating.value = (averageRating.value + rating) / 2;
};
...
const selectDate = (date) => {
  ...
  const slots = schedule.value[date];
  selectedSlot.value = slots && slots.length > 0 ? slots[0] : null;
};
